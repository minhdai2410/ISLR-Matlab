function h = biplot1(coefs,varargin)
% Chinh sua loi ve 'ObsLabels',obslabs
...
if ~isempty(varlabs)
    ...
    if in3D
        varTxtHndl = text(coefs(:,1)+delx,coefs(:,2)+dely,coefs(:,3)+delz,varlabs,'Interpreter','none');
    else
...
if ~isempty(obslabs)
    ...
    if in3D
        obsHndl = text(scores(:,1)+delx,scores(:,2)+dely,scores(:,3)+delz,obslabs,'Interpreter','none');
    else
        obsHndl = text(scores(:,1)+delx,scores(:,2)+dely,obslabs,'Interpreter','none');
    end
    set(obsHndl,'Tag','obslabels');
end
